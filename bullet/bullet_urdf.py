...
class UrdfEditor(object):
  ...
  def initializeFromBulletBody(self, bodyUid, physicsClientId):
    ...
    for j in range(p.getNumJoints(bodyUid, physicsClientId=physicsClientId)):
      ...
      self.jointNameToIndex[urdfJoint.joint_name] = len(self.urdfJoints)
      ...
      if self.linkParentToChild.get(urdfJoint.parent_name) is None:
        self.linkParentToChild[urdfJoint.parent_name] = []
      self.linkParentToChild[urdfJoint.parent_name].append(urdfJoint.joint_name)
